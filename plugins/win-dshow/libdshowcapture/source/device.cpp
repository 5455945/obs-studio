...
bool HDevice::SetupVideoCapture(IBaseFilter *filter, VideoConfig &config)
{
	...
	hr = InitCapturePicture();
	if (FAILED(hr)) {
		Error(L"InitCapturePicture Error!!!");
	}
	return true;
}
...
HRESULT HDevice::InitCapturePicture()
{
	HRESULT hr;

	hr = CoCreateInstance(CLSID_SampleGrabber, NULL, CLSCTX_INPROC_SERVER,
		IID_IBaseFilter, (LPVOID*)&sampleGrabberFilter);
	if (FAILED(hr)) {
		return hr;
	}

	hr = graph->AddFilter(sampleGrabberFilter, L"Sample Grabber Filter");
	if (FAILED(hr)) {
		return hr;
	}

	ComPtr<IBaseFilter> pNullFilter = NULL;
	hr = CoCreateInstance(CLSID_NullRenderer, NULL, CLSCTX_INPROC, IID_IBaseFilter, (void **)&pNullFilter);
	if (FAILED(hr)) {
		return hr;
	}

	hr = graph->AddFilter(pNullFilter, L"Null Render Filter");
	if (FAILED(hr)) {
		return hr;
	}

	hr = sampleGrabberFilter->QueryInterface(IID_ISampleGrabber, (void**)&pSampleGrabber);
	if (FAILED(hr)) {
		goto InitCapturePicture_End;
	}

	HDC hdc = GetDC(NULL);
	int iBitDepth = GetDeviceCaps(hdc, BITSPIXEL);
	sampleGrabberCallback.SetPictureBitCount(iBitDepth);
	ReleaseDC(NULL, hdc);
	...
	hr = builder->RenderStream(&PIN_CATEGORY_PREVIEW, &MEDIATYPE_Video, videoFilter, sampleGrabberFilter, pNullFilter);
	if (FAILED(hr)) {
		goto InitCapturePicture_End;
	}
	...
	sampleGrabberCallback.SetPictureWH(vih->bmiHeader.biWidth, vih->bmiHeader.biHeight);
	...
	hr = pSampleGrabber->SetCallback(&sampleGrabberCallback, 1);

InitCapturePicture_End:
...
bool HDevice::SaveCapturePicture(bool isSave)
{
	sampleGrabberCallback.SetSavePicture(isSave ? TRUE : FALSE);
	return true;
}

bool HDevice::SetPictureFileName(wchar_t* filename) {
	sampleGrabberCallback.SetPictureFileName(filename);
	return true;
}
