...
#define MyAppName "vhome"
#define MyAppVersion "1.1.15"
#define MyAppPublisher "SHANGHAI VANGEN NETWORK, Inc."
#define MyAppURL "http://www.vathome.cn/"
#define MyAppExeName "vhome.exe"
...
DefaultDirName={pf}\vhome\{#MyAppName}
...
;备注版本信息
InfoBeforeFile="vhome_license.txt"
InfoAfterFile="test_readme.txt"
VersionInfoCompany="http://www.vathome.cn/"
...
;[Components];Name: "delAppData"; Description: "删除历史配置信息"; Check: CheckAppDataVhome;

[Tasks]
Name: taskDelAppData; Description: "删除历史配置信息[{userappdata}\{#MyAppName}]"; GroupDescription: "配置信息:"; Check: CheckAppDataVhome; Flags:unchecked;
...
; 安装程序完成后，打开指定的网页
Filename: "http://www.vathome.cn/"; Description: "远程办公"; Flags: shellexec skipifsilent
...
[UninstallRun]
; 卸载完成后，打开指定页面
Filename: "http://www.vathome.cn/"; Flags: nowait shellexec;
...
var
  vhomepath: String;
  msg: String;// 卸载时，弹窗指定是否删除%appdata%\vhome
procedure CurUninstallStepChanged(CurUninstallStep: TUninstallStep);
begin
  if CurUninstallStep = usUninstall then
  begin
    vhomepath := ExpandConstant('{userappdata}');
    vhomepath := vhomepath + '\vhome';
    msg := '您是否要删除用户配置信息' + vhomepath + '?';
    if MsgBox(msg, mbConfirmation, MB_YESNO) = IDYES then
    begin
      DelTree(vhomepath, True, True, True);
    end;
  end;
end;
// 安装时，指定是否删除 %appdata%\vhome
function CheckAppDataVhome(): Boolean;
begin
    vhomepath := ExpandConstant('{userappdata}');
    vhomepath := vhomepath + '\vhome';
    Result := FileOrDirExists(vhomepath)
end;
