...
; Root: HKCU; Subkey: "Software\Microsoft\Windows\CurrentVersion\Run"; ValueType: string; ValueName: "xmf"; ValueData: "{app}\bin\32bit\{#MyAppName}32.exe"; Flags: uninsdeletekeyifempty
...
begin
    ...
    FWbemObjectSet := FWMIService.ExecQuery(Format('SELECT ExecutablePath FROM Win32_Process Where Name="%s"',[FileName]));
...
function OpenMyAppRunning(const FileName : string): Boolean;
var
    FSWbemLocator: Variant;
    FWMIService   : Variant;
    FWbemObjectSet: Variant;
...
begin
    ...
    FSWbemLocator := CreateOleObject('WBEMScripting.SWBEMLocator');
    FWMIService := FSWbemLocator.ConnectServer('', 'root\CIMV2', '', '');
    FWbemObjectSet := FWMIService.ExecQuery(Format('SELECT ExecutablePath, ProcessId, Handle, Name, Description, Caption FROM Win32_Process Where Name="%s"',[FileName]));
    Result := (FWbemObjectSet.Count > 0);
    ...
    begin
        ...
        begin
            //MsgBox(String(FWbemObject.ExecutablePath), mbInformation, MB_OK);
            //msg := '请先关闭正在运行的[' + FileName + ']程序';
            res := Exec(String(FWbemObject.ExecutablePath), 'kill_xmf', '', SW_SHOW, ewWaitUntilTerminated, ErrorCode);
            //if res = false then
            //begin
            //    msg := '安装程序将自动关闭正在运行的[' + FileName + ']程序';
...
begin
  ...
  OpenMyAppRunning('xmf32.exe');
  OpenMyAppRunning('xmf64.exe');
